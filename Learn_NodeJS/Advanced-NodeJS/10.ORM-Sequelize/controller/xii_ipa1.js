  ...
  controller.getAll = async (req, res, next) => {
	try {
		const result = await model.xii_ipa1.findAll({
			attributes: ["no","nama","alamat","hobi"],
			order: [["no","ASC"]],
			/*where: {
				no: {[Op.between]: [3,5]}
			}*/
...
controller.post = async function (req, res, next) {
	try {
		...
		const createNewRow = async (i) => {
			const result = await model.xii_ipa1.create({no: i, nama, hobi, alamat});
			res.status(201).json({
				status: "Post Method Success",
				message: "Data Siswa Has Been Added",
				newData: {no: i, nama, hobi, alamat}
			});
		};
...
controller.put = async function (req, res, next) {
    ...
    const result = await model.xii_ipa1.update({nama, hobi, alamat},{ 
      where: {no}
    });
    res.status(200).json({
      message: `Put Method Siswa No.${no} Successfully`,
      newData: {nama, hobi, alamat}
    });
